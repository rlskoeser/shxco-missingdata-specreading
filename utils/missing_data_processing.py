...
def process_events_data(events_df:pd.DataFrame) -> pd.DataFrame:
    ...
    This function processes the 'events' data by splitting multiple members for shared accounts and generating short IDs equivalent to those in the 'member' and 'book' DataFrames.
    ...
    """
    # Split the 'member_uris' column into 'first_member_uri' and 'second_member_uri' columns. This is done to handle shared accounts where multiple members are associated with a single event.
    events_df[["first_member_uri", "second_member_uri"]] = events_df.member_uris.str.split(";", expand=True)

    # Generate short IDs for members and items. The short ID is the second last part of the URI. For now, we only work with the first member for shared accounts.
    events_df["member_id"] = events_df.first_member_uri.apply(lambda x: x.split("/")[-2])
    events_df["item_id"] = events_df.item_uri.apply(lambda x: x.split("/")[-2] if pd.notna(x) else None)
...
def print_gaps(gaps: List[Dict[str, Union[datetime, int]]], gap_type: str):
    ...
    for gap in gaps:
        ...
        print(f"\t{gap['start'].strftime('%B %d %Y')} to {gap['end'].strftime('%B %d %Y')} ({gap['days']} days)")

def exclude_gap_events(events_df: pd.DataFrame, gaps: List[Dict[str, Union[datetime, int]]]) -> pd.DataFrame:
    """
    ...
    for gap in gaps:
        ...
        events_nogaps_df = events_nogaps_df[~((events_nogaps_df.logbook_date >= gap_start) & (events_nogaps_df.logbook_date <= gap_end))]
...
def generate_logbook_gaps(logbook_events_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    logbook_dates = pd.read_json(DATA_DIR / "logbook-dates.json").sort_values('startDate')
    ...
    for i in range(len(logbook_dates) - 1):
        ...
        if gap_duration > MIN_GAP_DAYS:
            logbook_gaps.append({'start': gap_start, 'end': gap_end, 'days': gap_duration})
        elif gap_duration > 0:  # Ignore 0 and -1 duration "gaps"!
            skipped_gaps.append({'start': gap_start, 'end': gap_end, 'days': gap_duration})
